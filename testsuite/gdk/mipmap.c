                ...
                GdkMemoryLayout       *out_layout)
{
  ...
  for (p = 0; p < gdk_memory_format_get_n_planes (src_layout->format); p++)
    {
      gsize block_width = gdk_memory_format_get_plane_block_width (src_layout->format, p);
      gsize block_height = gdk_memory_format_get_plane_block_height (src_layout->format, p);
      gsize block_bytes = gdk_memory_format_get_plane_block_bytes (src_layout->format, p);
              ...
              for (x = 0; x < src_layout->width; x++)
                {
                  for (xs = 0; xs < scale_x; xs)
                    {
                      memcpy (data + gdk_memory_layout_offset (out_layout, p, x * scale_x + xs, y * scale_y + ys),
                              src + gdk_memory_layout_offset (out_layout, p, x, y),
                              block_bytes);
                    }
                }
            }
